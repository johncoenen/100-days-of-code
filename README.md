...
Today's Progress: Created a basic ASP.NET MVC app
...
Today's Progress: Created a basic responsive web app with HTML5 and CSS Media Queries(from demo)
...
Today's Progress: Begin looking at jQuery UI, starting with draggable DOM elements
...
Today's Progress: Finished DOM tutorial called Understanding the DOM
...
Today's Progress: Began HTML Forms tutorial, with emphasis on how screen readers interpret forms
...
Today's Progress: Continue HTML forms tutorials looking at payment form and formfield input widgets
...
Today's Progress: Build ASP.NET Core Angular application from tutorial
...
Today's Progress: Added developer-friendly enhancements to ASP.NET Core Angular app

Thoughts

Proxy API calls to the ASP.NET Core Server using ng serve command -- this watches for changes to the code, transpiles the code, and reserves the code to localhost:4200 by default
Enabled automatic re-compilation for ASP.NET Core -- enables the ability to make changes to (server-side)code and show live changes in the already running Angular app
In order to run both applications in watch mode: -- First run: dotnet watch run (command) -- Then run: ng serve --proxy-config proxy.config.json(command in another terminal) -- Then browse to localhost:4200(default) to see live changes made to app
Link(s)
...
complete steps 1 - 4 of tutorial -- This app is data-driven and will enable the user to:
display a list of hero names
edit selected hero's details
navigate through different views of the hero's data
...
This is one of the more important sections of the Tutorial. I am going to go back to the beginning and write down the important areas of interest, up to this point, as a personal reference
Link
...
Turned the AppComponent into a navigation shell with links and a .
...
Link(s) https://github.com/johncoenen/CSS-Grid-with-Wes-Bos-Coursework
...
Link(s) https://www.shellscript.sh/index.html
...
Today's Progress: -Bootstrap continued - Glyphicons - Badges/Labels
...
Today's Progress: -Bootstrap continued - Progress Bars - Pagination - Pager
...
Today's Progress: -Bootstrap continued - List Group - Panels - Dropdowns
...
Links Shout out to Tyler McGinnis for the free course!
...
Links https://github.com/johncoenen/React_Bootcamp.git
...
https://www.codecademy.com/learn/learn-git Links
...
Links -- https://github.com/johncoenen/Web-Site-Examples
